...
var EXPORTED_SYMBOLS = ["SFDCCRUTCH.FileIO", "SFDCCRUTCH.DirIO"];

// Basic JavaScript File and Directory IO module
// By: MonkeeSage, v0.1
// Modified by: Matthieu Jamet
...
SFDCCRUTCH.FileIO = {
	...
	open   : function(path) {
		try {
		  ...
		  SFDCCRUTCH.Util.log(SFDCCRUTCH.Util.logC(),"open - EXCEPTION");
	...
	write  : function(file, data, mode, charset) {
      ...
      SFDCCRUTCH.Util.log(SFDCCRUTCH.Util.logC(),"write - EXCEPTION");
	...
	unlink : function(file) {
  	  ...
  	  SFDCCRUTCH.Util.log(SFDCCRUTCH.Util.logC(),"unlink - EXCEPTION");
			return false;
		}
	},
  unlinkfile : function(filename) {
		try {
      var file = SFDCCRUTCH.FileIO.open(filename);
      file.remove(false);
			return true;
		}
		catch(e) {
  	  SFDCCRUTCH.Util.log(SFDCCRUTCH.Util.logC(),"unlinkfile - EXCEPTION");
			return false;
		}
	},
	testfilesize : function(filename,size) {
		try {
      var file = SFDCCRUTCH.FileIO.open(filename);
   	  SFDCCRUTCH.Util.log(SFDCCRUTCH.Util.logA(),"testfilesize - opening "+filename);

      SFDCCRUTCH.FileIO.write(file," ");
      file.fileSize = size;
      file.remove(false);
			return true;
		}
		catch(e) {
      SFDCCRUTCH.Util.log(SFDCCRUTCH.Util.logC(),"testfilesize - EXCEPTION");
  	  return false;
		}
	},
	path   : function(file) {
...
//	'TmpD'				= OS tmp (e.g., /tmp)
SFDCCRUTCH.DirIO = {
	sep        : (Components.classes["@mozilla.org/xre/app-info;1"].getService(Components.interfaces.nsIXULRuntime).OS
	...
	open   : function(path) {
		return SFDCCRUTCH.FileIO.open(path);
	},
		...
		try {
			while (dirEntry.hasMoreElements()) {
				list.push(dirEntry.getNext()
								.QueryInterface(SFDCCRUTCH.FileIO.localfileIID));
			}
			if (recursive) {
				var list2 = new Array();
	...
	path   : function (dir) {
		return SFDCCRUTCH.FileIO.path(dir);
	},
